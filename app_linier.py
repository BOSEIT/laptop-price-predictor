...
from sklearn.linear_model import LinearRegression # <<< MODEL DIUBAH KE LINEAR REGRESSION
...
# from scipy import stats # scipy tidak dibutuhkan untuk LR, tapi biarkan saja jika ada analisis yang butuh

# --- Konfigurasi Halaman ---
st.set_page_config(
    page_title="Prediksi & Analisis Harga Laptop (Linear Regression) 💰",
    page_icon="💻",
    layout="wide",
...
# Faktor koreksi harga (64.55 di data Anda seharusnya 6.455 Juta IDR)
...
def train_model(data):
    ...
    # <<< PERUBAHAN UTAMA: Mengganti Random Forest dengan Linear Regression >>>
    model = Pipeline(steps=[('preprocessor', preprocessor),
                            ('regressor', LinearRegression())])
    # **********************************************************************
    
    # Melatih Model
    with st.spinner("⏳ Melatih Model Machine Learning (Linear Regression)..."):
...
if df is not None:
    ...
    st.title("💻 Aplikasi Prediksi Harga Laptop (Model Regresi Linier)")
    ...
    with pred_tab:
        ...
        with col_metric_2: st.metric(label="🧠 Model Digunakan", value="Linear Regression") # <<< Diubah
        ...
        if st.button("PREDIKSI HARGA", type="primary", use_container_width=True):
            with st.spinner('⏳ Model Regresi Linier sedang menghitung prediksi...'):
                time.sleep(1.5)
                # Outputnya sudah dalam Juta IDR yang dikoreksi (dibagi 10)
            ...
            with result_placeholder.container():
                ...
                st.caption(f"Perkiraan dihasilkan menggunakan model Regresi Linier berdasarkan data yang telah dikoreksi (dibagi {PRICE_CORRECTION_FACTOR}).")
    ...
    with anal_tab:
        ...
        st.info("Visualisasi ini membantu memahami hubungan linier antara spesifikasi laptop dan harga jualnya.")
        ...
        st.pyplot(fig_hist)
        plt.close(fig_hist) # Memastikan figure tertutup (mengatasi error Matplotlib)
        
        st.markdown("---")

        # 2. Scatter Plot RAM vs Harga (Sangat penting untuk Linear Regression)
        st.subheader("2. Korelasi RAM vs Harga")
        st.markdown("Melihat seberapa kuat hubungan linier antara peningkatan RAM dan harga jual.")
        
        fig_scatter, ax_scatter = plt.subplots(figsize=(10, 6))
        # Menggunakan regplot untuk menunjukkan garis regresi (hubungan linier)
        sns.regplot(x='RAM_GB', y=TARGET_COLUMN, data=df, ax=ax_scatter, scatter_kws={'alpha':0.6}, line_kws={'color':'red'})
        ax_scatter.set_title('Hubungan RAM vs Harga (Garis Regresi Linier)', fontsize=14)
        ax_scatter.set_xlabel('RAM (GB)')
        ax_scatter.set_ylabel('Harga (Juta IDR)')
        st.pyplot(fig_scatter)
        plt.close(fig_scatter) # Memastikan figure tertutup
        
        st.markdown("---")

        # 3. Box Plot GPU Type vs Harga
        st.subheader("3. Perbandingan Harga berdasarkan Tipe GPU")
        st.markdown("Membandingkan rata-rata harga laptop berdasarkan Tipe GPU.")
        ...
        st.pyplot(fig_box)
        plt.close(fig_box) # Memastikan figure tertutup
